...
const mk = (id: string, content: string, created_at: string, user_id = 'u1'): SyncMessage => ({
...
describe('messageSync.upsertRealtimeMessage', () => {
  ...
  it('replaces optimistic temp with server-confirmed message of same content', () => {
    ...
    let list = addOptimistic([] as SyncMessage[], temp);
  ...
  it('handles rapid resends of distinct optimistic messages without losing any', () => {
    ...
    // Server confirms all 5 in shuffled order
    [3, 0, 4, 2, 1].forEach((i) => {
...
describe('messageSync.mergeHistory (network drop recovery)', () => {
  it('preserves pending optimistic temps when re-fetching history after reconnect', () => {
    const history = [
      mk('s1', 'old1', '2026-06-03T09:00:00.000Z'),
      mk('s2', 'old2', '2026-06-03T09:00:01.000Z'),
    ];
    const pending: SyncMessage[] = [
      mk('temp-x', 'sent-during-outage', '2026-06-03T09:00:02.000Z'),
    ];
  ...
  it('drops optimistic temp if server already has the same content (avoid duplicate after reconnect)', () => {
    const pending: SyncMessage[] = [mk('temp-x', 'reconnected', '2026-06-03T09:00:02.000Z')];
    const history = [
      mk('s1', 'reconnected', '2026-06-03T09:00:02.100Z'),
    ];
    const merged = mergeHistory(pending, history);
  ...
  it('repeated history re-fetches are idempotent', () => {
    const history = [
      mk('s1', 'a', '2026-06-03T09:00:00.000Z'),
      mk('s2', 'b', '2026-06-03T09:00:01.000Z'),
    ];
    let list = mergeHistory([] as SyncMessage[], history);
...
describe('messageSync utility', () => {
  ...
  it('sortByCreated is stable and ascending', () => {
    const out = sortByCreated([
      mk('b', 'b', '2026-06-03T10:00:01.000Z'),
      mk('a', 'a', '2026-06-03T10:00:00.000Z'),
    ]);
...
describe('parseDevice', () => {
  ...
  it('detects iPad as Tablet', () => {
    const d = parseDevice(
      'Mozilla/5.0 (iPad; CPU OS 17_0 like Mac OS X) AppleWebKit/605.1 Mobile/15E148 Safari/604.1',
    );
