...
import { ArrowLeft, Calendar, Users, Sparkles, Loader2, AlertCircle, CheckCircle2, Clock, Plus } from 'lucide-react';
...
export default function ProjectDetailPage() {
  ...
  const load = async () => {
    if (!id) return;
    setLoading(true);
    const { data: p, error } = await supabase.from('projects' as any).select('*').eq('id', id).maybeSingle();
    if (error || !p) { setLoading(false); return; }
    setProject(p);

    if ((p as any).client_id) {
      const { data: c } = await supabase.from('clients').select('*').eq('id', (p as any).client_id).maybeSingle();
      setClient(c);
    }

    const { data: mem } = await supabase.from('project_members' as any).select('*').eq('project_id', id);
    if (mem && (mem as any[]).length > 0) {
      const userIds = (mem as any[]).map(m => m.user_id);
      const { data: profiles } = await supabase.from('profiles').select('id, full_name, avatar_url').in('id', userIds);
      setMembers((profiles || []).map(pr => ({
        ...pr,
        role: (mem as any[]).find(m => m.user_id === pr.id)?.role || 'member',
      })));
    } else {
      setMembers([]);
    }
  ...
  const updateStatus = async (newStatus: string) => {
    if (!project) return;
    const { error } = await supabase.from('projects' as any).update({ status: newStatus } as any).eq('id', project.id);
  ...
  if (!project) return (
    <div className="text-center py-16 space-y-3">
      ...
      <Button variant="outline" onClick={() => navigate('/projects')}><ArrowLeft className="h-4 w-4 mr-1.5" /> Back</Button>
    </div>
  );
        ...
        <CardContent className="p-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge variant="outline" className={statusVariants[project.status]}>{project.status.replace('_', ' ')}</Badge>
              </div>
              {client && (
          ...
          {project.service_types?.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {project.service_types.map((s: string) => (
                    ...
                    <li key={a.id} className="text-sm flex items-start gap-2 py-1.5 border-b border-border last:border-0">
                      ...
                      <div className="flex-1">
                        ...
                        {a.metadata && Object.keys(a.metadata).length > 0 && (
                          <span className="text-muted-foreground"> · {JSON.stringify(a.metadata).slice(0, 80)}</span>
                        )}
                      </div>
