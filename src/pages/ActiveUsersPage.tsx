...
import { Shield, Search, Wifi, WifiOff, RefreshCw, Users, Clock } from 'lucide-react';
import { format, formatDistanceToNow } from 'date-fns';

type Profile = { id: string; full_name: string | null; avatar_url: string | null; department: string | null };
type SessionInfo = { user_id: string; last_active_at: string; device_info: string | null; created_at: string };
...
const ActiveUsersPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setSessions((sessionsRes.data as SessionInfo[]) || []);
  ...
  // No longer track own session here - it's done in DashboardLayout
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight" style={{ lineHeight: '1.2' }}>Active Users</h1>
                  ...
                  <div key={member.id} className="flex items-center gap-3 p-3 rounded-lg bg-success/5 border border-success/10">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <Badge variant="secondary" className={`text-[9px] px-1 py-0 ${roleColors[memberRole]}`}>{memberRole}</Badge>
                      </div>
