...
const statusLabels: Record<string, string> = {
  active: 'Active', on_hold: 'On Hold', in_review: 'In Review',
...
export default function ProjectsPage() {
  ...
  const exportProjects = () => {
    exportToCsv('projects', filtered.map(p => ({
      Name: p.name,
  ...
  const logDeletions = async (items: Project[]) => {
  ...
  const exportSelected = () => {
    exportToCsv('projects-selected', selectedProjects.map(p => ({
      Name: p.name,
      Client: p.client_id ? clients[p.client_id] || '' : '',
      Status: statusLabels[p.status] || p.status,
      Progress: `${p.progress}%`,
          ...
          <CardContent className="py-16 flex flex-col items-center text-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {isAdmin ? 'Create a client to auto-launch a project.' : 'Projects assigned to you will appear here.'}
              </p>
            </div>
            {isAdmin && <Button onClick={() => setShowWizard(true)}><Plus className="h-4 w-4 mr-1.5" /> New Client</Button>}
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map(p => {
            ...
            return (
              <Card key={p.id} className={`hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer group ${selectedIds.has(p.id) ? 'ring-2 ring-primary' : ''}`}
                ...
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {p.client_id && clients[p.client_id] && (
                        <p className="text-xs text-muted-foreground truncate">{clients[p.client_id]}</p>
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    {p.deadline ? (
                      <div className={`flex items-center gap-1 ${
                        days !== null && days < 0 ? 'text-destructive' : days !== null && days < 7 ? 'text-warning' : 'text-muted-foreground'
                      }`}>
                        <Calendar className="h-3 w-3" />
                        {days !== null && days < 0 ? `${Math.abs(days)}d overdue` :
                         days === 0 ? 'Due today' :
                         days !== null ? `${days}d left` : 'No deadline'}
                      </div>
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the selected projects and all their tasks. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting}>Cancel</AlertDialogCancel>
