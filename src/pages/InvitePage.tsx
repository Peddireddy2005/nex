...
const InvitePage = () => {
  ...
  const [invite, setInvite] = useState<{ role: string; email: string | null } | null>(null);
  ...
  const checkToken = async () => {
    ...
    const row = data[0];
    if (row.used) { setStatus('used'); return; }
    if (row.expired) { setStatus('expired'); return; }
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (authData.user) {
      // Use server-side edge function to apply role securely
