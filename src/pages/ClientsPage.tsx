...
const SERVICE_TYPES = [
  'AI Ads', 'Automation', 'Website Development', 'Mobile App',
  'Branding', 'Social Media', 'SEO', 'Consulting', 'General',
];
...
const ClientsPage = () => {
  ...
  useEffect(() => { fetchClients(); fetchProjects(); }, []);

  const resetForm = () => {
    setName(''); setEmail(''); setPhone(''); setCompany('');
    setStatus('active'); setServiceType('General'); setNotes('');
    setEditingClient(null); setAutoGenerate(true);
  };
  ...
  const saveClient = async () => {
    ...
    try {
      const payload = {
        ...
        company: company || null, status, notes: notes || null,
        service_type: serviceType,
      };

      if (editingClient) {
        await supabase.from('clients').update({ ...payload, updated_at: new Date().toISOString() } as any).eq('id', editingClient.id);
  ...
  const deleteClient = async (id: string) => {
    if (role !== 'admin') { toast.error('Only admins can delete clients'); return; }
  ...
  const openEdit = (c: Client) => {
    if (role !== 'admin') { toast.error('Only admins can edit clients'); return; }
    setEditingClient(c);
    setName(c.name); setEmail(c.email || ''); setPhone(c.phone || '');
    setCompany(c.company || ''); setStatus(c.status);
    setServiceType(c.service_type || 'General'); setNotes(c.notes || '');
    setShowCreate(true);
  };

  const getClientProjects = (clientId: string) => projects.filter(p => p.client_id === clientId);
  const getProjectProgress = (clientId: string) => {
  ...
  return (
    <div className="space-y-6">
        ...
        {isAdmin && (
          <Dialog open={showCreate} onOpenChange={(o) => { setShowCreate(o); if (!o) resetForm(); }}>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader><DialogTitle>{editingClient ? 'Edit Client' : 'Onboard New Client'}</DialogTitle></DialogHeader>
              <div className="space-y-3">
                ...
                <Button onClick={saveClient} className="w-full" disabled={saving}>
                  {saving ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Processing...</> : editingClient ? 'Update' : 'Create & Launch Project'}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <Card><CardContent className="py-4 text-center"><p className="text-2xl font-bold">{clients.length}</p><p className="text-xs text-muted-foreground">Total Clients</p></CardContent></Card>
        <Card><CardContent className="py-4 text-center"><p className="text-2xl font-bold">{clients.filter(c => c.status === 'active').length}</p><p className="text-xs text-muted-foreground">Active</p></CardContent></Card>
        <Card><CardContent className="py-4 text-center"><p className="text-2xl font-bold">{clients.filter(c => c.status === 'lead').length}</p><p className="text-xs text-muted-foreground">Leads</p></CardContent></Card>
        <Card><CardContent className="py-4 text-center"><p className="text-2xl font-bold">{projects.length}</p><p className="text-xs text-muted-foreground">Projects</p></CardContent></Card>
      </div>

      <Tabs defaultValue="clients">
        <TabsList><TabsTrigger value="clients">Clients</TabsTrigger><TabsTrigger value="projects">Projects</TabsTrigger></TabsList>
        <TabsContent value="clients">
          {loading ? <p className="text-muted-foreground">Loading...</p> : clients.length === 0 ? (
            <Card><CardContent className="py-8 text-center text-muted-foreground">No clients yet. Create your first client to start the delivery pipeline.</CardContent></Card>
          ) : (
            <Card>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {clients.map(c => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell>
                        <Badge variant="secondary" className="text-xs">{c.service_type || 'General'}</Badge>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          {getClientProjects(c.id).length > 0 && (
                            <Progress value={getProjectProgress(c.id)} className="w-16 h-1.5" />
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          {isAdmin && (
                            <>
                              <Button size="sm" variant="ghost" onClick={() => openEdit(c)}><Edit2 className="h-3 w-3" /></Button>
                              <Button size="sm" variant="ghost" onClick={() => deleteClient(c.id)}><Trash2 className="h-3 w-3 text-destructive" /></Button>
                            </>
                          )}
                        </div>
                      </TableCell>
