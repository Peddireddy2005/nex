...
const PerformancePage = () => {
  ...
  const [leaderboard, setLeaderboard] = useState<UserPerf[]>([]);
  ...
  const fetchPerformance = async () => {
    ...
    try {
      const [profilesRes, tasksRes, ratingsRes, assigneesRes, historyRes] = await Promise.all([
        ...
        supabase.from('task_completion_history' as any).select('*').order('completed_at', { ascending: false }),
      ]);

      const profilesList = profilesRes.data;
      const tasks = tasksRes.data;
      const ratingsData = ratingsRes.data as RatingRecord[] | null;
      const assignees = assigneesRes.data;
      const history = (historyRes.data || []) as unknown as CompletionRecord[];

      if (!profilesList) { setLoading(false); return; }

      const nameMap: Record<string, string> = {};
      profilesList.forEach(p => { nameMap[p.id] = p.full_name || 'Unknown'; });
      setProfiles(nameMap);
      ...
      profilesList.forEach((p) => {
        perfMap[p.id] = {
          ...
          completedCount: 0, totalAssigned: 0, avgRating: 0, ratingCount: 0,
        };
      });

      // Count from completion history for accurate completed count
      history.forEach(h => {
      ...
      // Add completed count to totalAssigned for accurate rate
  ...
  const getCompletionRate = (m: UserPerf) =>
    m.totalAssigned > 0 ? Math.round((m.completedCount / m.totalAssigned) * 100) : 0;
  ...
  const avgTeamRating = leaderboard.filter(m => m.avgRating > 0).length > 0
  ...
  if (loading) {
    return (
      <div className="space-y-6 animate-fade-in-up">
        <h1 className="text-2xl font-bold tracking-tight" style={{ lineHeight: '1.2' }}>Performance</h1>
        ...
        {isAdminOrManager && (
          <Dialog open={rateOpen} onOpenChange={setRateOpen}>
            <DialogTrigger asChild>
              <Button size="sm"><Star className="h-4 w-4 mr-1" />Rate Member</Button>
            </DialogTrigger>
            <DialogContent className="max-w-sm">
              <DialogHeader><DialogTitle>Rate Team Member</DialogTitle></DialogHeader>
              <form onSubmit={submitRating} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select value={selectedUser} onValueChange={setSelectedUser}>
                    ...
                    <SelectContent>
                      {leaderboard.filter((m) => m.id !== user?.id).map((m) => (
                ...
                <Button type="submit" className="w-full" disabled={!selectedUser}>Submit Rating</Button>
              </form>
            </DialogContent>
          </Dialog>
        )}
        ...
        <Card className="shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="p-1.5 rounded-md bg-primary/10"><Users className="h-4 w-4 text-primary" /></div>
            </div>
        ...
        <Card className="shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="p-1.5 rounded-md bg-success/10"><Target className="h-4 w-4 text-success" /></div>
            </div>
            <div className="text-2xl font-bold tabular-nums">{totalCompleted}<span className="text-sm font-normal text-muted-foreground">/{totalAssigned}</span></div>
          </CardContent>
        </Card>
        <Card className="shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="p-1.5 rounded-md bg-info/10"><BarChart3 className="h-4 w-4 text-info" /></div>
            </div>
        ...
        <Card className="shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="p-1.5 rounded-md bg-accent/10"><Star className="h-4 w-4 text-accent" /></div>
            </div>
            <div className="text-2xl font-bold tabular-nums">{avgTeamRating || '—'}<span className="text-sm font-normal text-muted-foreground">/10</span></div>
          </CardContent>
        </Card>
      </div>
              ...
              return (
                <Card key={member.id} className="shadow-sm hover:shadow-md transition-[box-shadow]">
                  <CardContent className="py-3 px-4 flex items-center gap-3">
                    <div className="w-7 text-center font-bold text-sm tabular-nums text-muted-foreground">
                      {i < 3 ? <Trophy className={`h-4 w-4 mx-auto ${medalColors[i]}`} /> : `#${i + 1}`}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-0.5">
                        <span className="text-xs text-muted-foreground">{member.completedCount}/{member.totalAssigned} tasks</span>
                        ...
                        <div className="flex items-center gap-2">
                          <Avatar className="h-7 w-7">
                            <AvatarFallback className="bg-primary/10 text-primary text-[10px]">
                              {getInitials(profiles[record.user_id] || null)}
                            </AvatarFallback>
                          </Avatar>
                      ...
                      <TableCell>
                        <div>
                          ...
                          {record.task_description && (
                            <p className="text-xs text-muted-foreground line-clamp-1">{record.task_description}</p>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Badge variant="secondary" className="text-[10px]">
                          {record.was_group_task ? (
                            <span className="flex items-center gap-1"><Users className="h-3 w-3" />Group ({record.group_members?.length || 0})</span>
                          ) : 'Individual'}
                        </Badge>
                      </TableCell>
                      <TableCell>
              ...
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    ...
                    {ratings.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No ratings yet</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
