...
type Profile = { id: string; full_name: string | null; avatar_url: string | null; department: string | null };
type DeviceSetting = { user_id: string; allow_multi_device: boolean };
type SessionRow = {
  id: string;
  user_id: string;
  session_id: string;
...
const SessionManagementPage = () => {
  ...
  const fetchMySessions = async () => {
    ...
    setMySessions((data as SessionRow[]) || []);
  };
  ...
  const fetchAdminData = async () => {
    ...
    ((settingsRes.data as DeviceSetting[]) || []).forEach(s => { sMap[s.user_id] = s.allow_multi_device; });
    setSettings(sMap);
    const rMap: Record<string, string> = {};
    (rolesRes.data || []).forEach(r => { rMap[r.user_id] = r.role; });
    setRoles(rMap);
  };
  ...
  // Heuristic for "current session": the most-recently-active row.
  useEffect(() => {
  ...
  const toggleMultiDevice = async (userId: string, allow: boolean) => {
    const existing = settings[userId] !== undefined;
    if (existing) {
      const { error } = await supabase.from('user_device_settings').update({ allow_multi_device: allow, updated_at: new Date().toISOString() } as any).eq('user_id', userId);
      if (error) { toast.error(error.message); return; }
    } else {
      const { error } = await supabase.from('user_device_settings').insert({ user_id: userId, allow_multi_device: allow } as any);
      if (error) { toast.error(error.message); return; }
    }
  ...
  const filtered = profiles.filter(p =>
    !search || p.full_name?.toLowerCase().includes(search.toLowerCase()) || p.department?.toLowerCase().includes(search.toLowerCase())
  );

  return (
    <div className="space-y-6 animate-fade-in-up">
      <div>
        <h1 className="text-2xl font-bold tracking-tight" style={{ lineHeight: '1.2' }}>Session Management</h1>
        <p className="text-muted-foreground mt-1 text-sm">Review the devices signed in to your account.</p>
      </div>

      {/* My Devices — visible to everyone */}
      <Card className="shadow-sm">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {mySessions.length === 0
                ...
                : `${mySessions.length} active session${mySessions.length === 1 ? '' : 's'}.`}
            </p>
          </div>
        </CardHeader>
        <CardContent className="space-y-1">
              ...
              <div key={s.id} className="flex items-center gap-3 py-3 px-2 rounded-lg hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isCurrent && <Badge variant="secondary" className="text-[10px] bg-success/10 text-success">This device</Badge>}
                  </div>
          ...
          {mySessions.length === 0 && (
            <p className="text-center text-sm text-muted-foreground py-8">No sessions to display.</p>
          )}
        </CardContent>
      </Card>

      {/* Admin: per-user multi-device toggle */}
      {role === 'admin' && (
        <>
          <Separator />
                  ...
                  <div key={member.id} className="flex items-center gap-3 py-3 px-2 rounded-lg hover:bg-muted/50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="secondary" className={`text-[10px] ${roleColors[memberRole]}`}>{memberRole}</Badge>
                      </div>
