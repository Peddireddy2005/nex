...
type TaskComment = { id: string; content: string; user_id: string; created_at: string };
type Profile = { id: string; full_name: string | null };
type TaskAssignee = { id: string; task_id: string; user_id: string };
...
const statusColors: Record<string, string> = { todo: 'bg-muted text-muted-foreground', in_progress: 'bg-info/10 text-info', review: 'bg-warning/10 text-warning', completed: 'bg-success/10 text-success' };
const statusColumnColors: Record<string, string> = { todo: 'border-t-muted-foreground/30', in_progress: 'border-t-info/60', review: 'border-t-warning/60', completed: 'border-t-success/60' };
...
const TasksPage = () => {
  ...
  useEffect(() => { fetchTasks(); fetchProfiles(); fetchAllAssignees(); }, [fetchTasks]);
  ...
  const resetForm = () => {
    ...
    setSelectedAssignees([]); setDeadline(''); setEditingTask(null);
  };
  ...
  const canDeleteTask = (task: Task) => isAdminOrManager || task.created_by === user?.id;
  ...
  const updateTaskStatus = async (taskId: string, newStatus: string) => {
    ...
    // When completed, record history for all assignees and unassign them
    if (newStatus === 'completed') {
      ...
      if (allMembers.length > 0) {
        ...
        await supabase.from('task_completion_history' as any).insert(historyRecords);
      }

      // Unassign all members on completion
      await supabase.from('task_assignees').delete().eq('task_id', taskId);
      await supabase.from('tasks').update({ assigned_to: null } as any).eq('id', taskId);
      fetchAllAssignees();
    }
  ...
  const isAssignedToUser = (taskId: string) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else { toast.success('File uploaded'); fetchAttachments(detailTask.id); }
    setUploading(false);
  };
  ...
  const renderAssigneeAvatars = (taskId: string) => {
    const assignees = taskAssignees[taskId] || [];
    if (assignees.length === 0) {
      return (
        <div className="flex items-center gap-1.5">
          <Avatar className="h-5 w-5"><AvatarFallback className="text-[8px]">??</AvatarFallback></Avatar>
          ...
          {assignees.slice(0, 3).map(uid => (
            <Avatar key={uid} className="h-5 w-5 border-2 border-background">
              <AvatarFallback className="text-[8px] bg-primary/10 text-primary">
                {getInitials(getProfileName(uid) === 'Unassigned' ? null : getProfileName(uid))}
              </AvatarFallback>
            </Avatar>
          ))}
        </div>
        {assignees.length > 3 && (
          <span className="text-[10px] text-muted-foreground ml-1">+{assignees.length - 3}</span>
        )}
        {assignees.length <= 2 && (
          <span className="text-[11px] text-muted-foreground ml-1 truncate max-w-[80px]">
            {assignees.map(id => getProfileName(id)).join(', ')}
          </span>
        )}
      </div>
    );
  ...
  const renderTaskCard = (task: Task, index: number, draggable = true) => {
    const inner = (
      <Card className="shadow-sm hover:shadow-md transition-[box-shadow] cursor-pointer active:scale-[0.98] border border-border/50" onClick={() => openDetail(task)}>
        <CardContent className="p-3 space-y-2">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <Badge variant="secondary" className={`text-[10px] px-1.5 py-0 ${priorityColors[task.priority]}`}>{task.priority}</Badge>
    ...
    return (
      <Draggable key={task.id} draggableId={task.id} index={index}>
        {(provided) => (
          <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className="mb-2">{inner}</div>
        )}
      </Draggable>
    );
  };
  ...
  return (
    <div className="space-y-4 animate-fade-in-up">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight" style={{ lineHeight: '1.2' }}>Tasks</h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="outline" size="sm" onClick={() => exportToCsv('tasks', tasks.map(t => ({
            Title: t.title,
                  ...
                  <div className="border rounded-md p-2 max-h-40 overflow-y-auto space-y-1">
                    {profiles.map(p => (
                      <label key={p.id} className="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-muted/50 cursor-pointer">
                        ...
                        <Avatar className="h-5 w-5"><AvatarFallback className="text-[8px] bg-primary/10 text-primary">{getInitials(p.full_name)}</AvatarFallback></Avatar>
                        <span className="text-sm">{p.full_name || 'Unknown'}</span>
                      </label>
                    ))}
                  </div>
                  {selectedAssignees.length > 0 && (
                    <p className="text-xs text-muted-foreground">{selectedAssignees.length} member{selectedAssignees.length > 1 ? 's' : ''} selected</p>
                  )}
                </div>

                <div className="space-y-2"><Label>Deadline</Label><Input type="date" value={deadline} onChange={e => setDeadline(e.target.value)} /></div>
                <Button type="submit" className="w-full">{editingTask ? 'Update' : 'Create'} Task</Button>
              </form>
            </DialogContent>
          </Dialog>
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          ...
          <SelectContent><SelectItem value="all">All Status</SelectItem>{STATUSES.map(s => <SelectItem key={s} value={s}>{statusLabels[s]}</SelectItem>)}</SelectContent>
        </Select>
      ...
      {viewMode === 'kanban' ? (
            ...
            {STATUSES.map(s => (
              <div key={s} className={`rounded-lg bg-muted/40 border-t-2 ${statusColumnColors[s]} p-3 min-h-[200px]`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Badge variant="secondary" className="text-[10px] tabular-nums">{tasksByStatus[s].length}</Badge>
                </div>
                <Droppable droppableId={s}>
                  {(provided, snapshot) => (
                    <div ref={provided.innerRef} {...provided.droppableProps}
                      className={`min-h-[120px] rounded-md transition-colors ${snapshot.isDraggingOver ? 'bg-primary/5' : ''}`}>
        ...
        <div className="space-y-2">
          {tasks.length === 0 ? (
            <Card className="shadow-sm"><CardContent className="py-12 text-center text-muted-foreground"><p>No tasks found. Create your first task.</p></CardContent></Card>
          ) : tasks.map(task => (
            <Card key={task.id} className="shadow-sm hover:shadow-md transition-[box-shadow] cursor-pointer active:scale-[0.99]" onClick={() => openDetail(task)}>
              <CardContent className="py-3 px-4 flex items-center gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <Badge variant="secondary" className={`text-[10px] ${statusColors[task.status]}`}>{statusLabels[task.status]}</Badge>
                  {canDeleteTask(task) && (
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); setDeleteConfirm(task); }}
            ...
            <>
              ...
              <ScrollArea className="flex-1 -mx-6 px-6">
                <div className="space-y-4 pb-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <Badge variant="secondary" className={priorityColors[detailTask.priority]}>{detailTask.priority}</Badge>
                    <Badge variant="secondary" className={statusColors[detailTask.status]}>{statusLabels[detailTask.status]}</Badge>
                  </div>

                  {/* Status update for assigned members */}
                  ...
                  <div className="space-y-1 text-sm">
                        ...
                        {(taskAssignees[detailTask.id] || []).map(uid => (
                          <Avatar key={uid} className="h-6 w-6 border-2 border-background">
                            <AvatarFallback className="text-[9px] bg-primary/10 text-primary">
                              {getInitials(getProfileName(uid))}
                            </AvatarFallback>
                          </Avatar>
                        ))}
                      </div>
                    ...
                    {detailTask.deadline && (
                      <div><span className="text-muted-foreground">Deadline:</span> <span className="font-medium">{new Date(detailTask.deadline).toLocaleDateString()}</span></div>
                    )}
                  </div>

                  <div className="flex gap-2 flex-wrap">
                    <Button variant="outline" size="sm" asChild>
                      <Link to={`/tasks/${detailTask.id}/documents`}><FolderOpen className="h-3 w-3 mr-1" />Documents</Link>
                    </Button>
                    {(isAdminOrManager || detailTask.created_by === user?.id) && (
                      <>
                        <Button variant="outline" size="sm" onClick={() => openEdit(detailTask)}>Edit</Button>
                        <Button variant="destructive" size="sm" onClick={() => setDeleteConfirm(detailTask)}><Trash2 className="h-3 w-3 mr-1" />Delete</Button>
                      </>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-semibold flex items-center gap-1.5"><Paperclip className="h-3.5 w-3.5" />Attachments</h4>
                    ...
                    {attachments.length === 0 ? (
                      ...
                      <div className="space-y-1">
                        {attachments.map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block text-xs text-primary hover:underline truncate">{decodeURIComponent(url.split('/').pop() || '')}</a>
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="text-sm font-semibold flex items-center gap-1.5"><MessageSquare className="h-3.5 w-3.5" />Comments</h4>
                    ...
                    <div className="flex gap-2">
                      <Input value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Add a comment..."
                        className="flex-1 h-9" onKeyDown={e => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), addComment())} />
                      <Button size="sm" onClick={addComment} disabled={!newComment.trim()}><Send className="h-3.5 w-3.5" /></Button>
                    </div>
                  </div>
                </div>
              </ScrollArea>
