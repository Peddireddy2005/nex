...
type Meeting = {
  ...
  duration_minutes: number; meeting_link: string | null; created_by: string; created_at: string;
};
type Attendee = { id: string; meeting_id: string; user_id: string; status: string };
...
const SchedulerPage = () => {
  ...
  const fetchMeetings = async () => {
    const { data } = await (supabase as any).from('meetings').select('*').order('meeting_date', { ascending: true });
    setMeetings(data || []);
  };

  const fetchAttendees = async () => {
    const { data } = await (supabase as any).from('meeting_attendees').select('*');
  ...
  useEffect(() => { fetchMeetings(); fetchAttendees(); fetchProfiles(); }, []);

  const resetForm = () => {
    setTitle(''); setDescription(''); setMeetingDate(''); setMeetingTime('10:00');
    setDuration('30'); setMeetingLink(''); setSelectedAttendees([]);
  };
  ...
  const createMeeting = async (e: React.FormEvent) => {
    ...
    const { data, error } = await (supabase as any).from('meetings').insert({
      ...
      meeting_date: dateTime, duration_minutes: parseInt(duration),
    ...
    if (selectedAttendees.length > 0 && data) {
      ...
      await (supabase as any).from('meeting_attendees').insert(inserts);
    }
    toast.success('Meeting scheduled!');
    resetForm(); setCreateOpen(false);
    fetchMeetings(); fetchAttendees();
  };
  ...
  const updateMeeting = async (e: React.FormEvent) => {
    ...
    if (error) { toast.error(error.message); return; }
    // Update attendees
    await (supabase as any).from('meeting_attendees').delete().eq('meeting_id', editingMeeting.id);
    if (selectedAttendees.length > 0) {
      await (supabase as any).from('meeting_attendees').insert(
        selectedAttendees.map(uid => ({ meeting_id: editingMeeting.id, user_id: uid }))
      );
    }
    toast.success('Meeting updated!');
    resetForm(); setEditOpen(false); setEditingMeeting(null);
    fetchMeetings(); fetchAttendees();
  };
  ...
  const deleteMeeting = async (id: string) => {
    ...
    if (error) toast.error(error.message); else { toast.success('Meeting deleted'); fetchMeetings(); }
  };

  const createInstantMeeting = () => {
    // Generate a Google Meet-style link (opens Google Meet new meeting page)
  ...
  const renderMeetingForm = (onSubmit: (e: React.FormEvent) => void, submitLabel: string) => (
    <form onSubmit={onSubmit} className="space-y-4 pt-2 max-h-[70vh] overflow-y-auto">
      ...
      <div className="space-y-2">
        <Label className="flex items-center gap-1.5"><Link2 className="h-3.5 w-3.5" /> Meeting Link</Label>
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight" style={{ lineHeight: '1.2' }}>Scheduler</h1>
            ...
            <>
              ...
              <Dialog open={createOpen} onOpenChange={o => { setCreateOpen(o); if (!o) resetForm(); }}>
                <DialogTrigger asChild>
                  <Button size="sm" className="gap-1.5"><Plus className="h-4 w-4" /> Schedule</Button>
                </DialogTrigger>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {past && <Badge variant="secondary" className="text-[10px] shrink-0">Past</Badge>}
                    </div>
      ...
      {/* Edit Meeting Dialog */}
      <Dialog open={editOpen} onOpenChange={o => { setEditOpen(o); if (!o) { resetForm(); setEditingMeeting(null); } }}>
        <DialogContent className="sm:max-w-lg">
