...
import {
  Save, Camera, Loader2, User, Palette, Shield, LogOut, Mail, Building2,
  Sun, Moon, Monitor, RotateCcw, Check, Sparkles, Copy,
} from 'lucide-react';
...
const SettingsPage = () => {
  ...
  const dirty =
    ...
    department.trim() !== initial.department.trim();
  ...
  const themeOptions: { value: 'light' | 'dark' | 'system'; label: string; icon: any; preview: string }[] = [
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in-up pb-20">
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 mt-1.5 text-sm text-muted-foreground">
              ...
              {initial.department && (
                <span className="inline-flex items-center gap-1.5"><Building2 className="h-3.5 w-3.5" />{initial.department}</span>
              )}
            </div>
            <button
              type="button"
      ...
      <Tabs value={tab} onValueChange={onTabChange} className="space-y-4">
        <TabsList className="grid grid-cols-3 w-full sm:w-fit">
          <TabsTrigger value="profile" className="gap-1.5"><User className="h-3.5 w-3.5" />Profile{dirty && <span className="ml-1 h-1.5 w-1.5 rounded-full bg-warning animate-pulse" />}</TabsTrigger>
            ...
            <Card className="lg:sticky lg:top-20 h-fit">
              ...
              <CardContent>
                <div className="rounded-2xl border border-border/60 bg-gradient-to-br from-card to-muted/30 p-4">
                  ...
                  <p className="text-xs text-muted-foreground mt-3 line-clamp-4 min-h-[3rem]">
                    {bio || <span className="italic">Your bio will appear here…</span>}
                  </p>
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <div className="rounded-xl border border-border/60 bg-muted/20 p-4">
                    ...
                    <p className="text-sm font-medium capitalize mt-1 inline-flex items-center gap-1.5">
                      <Shield className="h-3.5 w-3.5 text-primary" />{role || 'member'}
                    </p>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/20 p-4">
      ...
      {/* Sticky save bar when dirty (Profile tab) */}
      {dirty && (
        <div className="fixed bottom-4 inset-x-4 sm:left-auto sm:right-6 sm:bottom-6 z-40 animate-fade-in-up">
          <div className="mx-auto sm:mx-0 max-w-md rounded-2xl border border-border/60 bg-card/90 backdrop-blur-xl shadow-2xl p-3 flex items-center gap-3">
            ...
            <Button variant="ghost" size="sm" onClick={handleReset} disabled={saving}>Discard</Button>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You've edited your profile but haven't saved yet. What would you like to do?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel onClick={handleCancelGuard}>Stay on page</AlertDialogCancel>
            <Button variant="ghost" onClick={handleConfirmDiscard} disabled={saving}>Discard changes</Button>
