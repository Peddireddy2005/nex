...
type UserOnboarding = { step_id: string; completed: boolean };
...
const OnboardingPage = () => {
  ...
  useEffect(() => { fetchSteps(); fetchProgress(); fetchProfiles(); fetchCustomRoles(); }, [user, role]);
  ...
  const addMember = async () => {
    ...
    try {
      ...
      if (res.error) {
        ...
        setMemberEmail(''); setMemberPassword(''); setMemberName('');
        setMemberDepartment(''); setMemberBio(''); setMemberRole('member'); setMemberPhone('');
        setMemberCustomRoleId('none'); setAddMemberOpen(false);
        fetchProfiles();
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight" style={{ lineHeight: '1.2' }}>Onboarding</h1>
          <p className="text-muted-foreground mt-1 text-sm">
            {profile && <Badge variant="secondary" className={`mr-2 ${statusColors[profile.onboarding_status]}`}>{profile.onboarding_status}</Badge>}
                ...
                <DialogContent className="sm:max-w-lg">
                  ...
                  <div className="space-y-4 pt-2 max-h-[70vh] overflow-y-auto">
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      The member will be able to log in immediately with these credentials.
                    </p>
                  </div>
                </DialogContent>
              </Dialog>
