...
type Step = {
  ...
  order_index: number;
  default_status: 'pending' | 'training' | 'active';
};
...
export const OnboardingStepsEditor = () => {
  ...
  const [newStatus, setNewStatus] = useState<Step['default_status']>('pending');
  ...
  const updateStep = async (id: string, patch: Partial<Step>) => {
    const { error } = await supabase.from('onboarding_steps').update(patch as any).eq('id', id);
    if (error) { toast.error(error.message); return; }
    toast.success('Saved');
    load();
  };
  ...
  const move = async (idx: number, dir: -1 | 1) => {
    ...
    await supabase.from('onboarding_steps').update({ order_index: b.order_index } as any).eq('id', a.id);
    await supabase.from('onboarding_steps').update({ order_index: a.order_index } as any).eq('id', b.id);
    load();
  };

  return (
    <Card className="shadow-sm">
      <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold">Manage Onboarding Steps</CardTitle></CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2 p-3 border rounded-md bg-muted/30">
          ...
          <div className="flex gap-2">
            <Select value={newStatus} onValueChange={(v) => setNewStatus(v as Step['default_status'])}>
            ...
            <Button size="sm" onClick={addStep} className="gap-1"><Plus className="h-4 w-4" /> Add</Button>
          </div>
        </div>

        {loading ? <p className="text-sm text-muted-foreground">Loading…</p> : steps.length === 0 ? (
    ...
    <div className="p-3 border rounded-md space-y-2">
      <div className="flex gap-2 items-start">
        <div className="flex flex-col gap-1">
          <Button size="icon" variant="outline" className="h-7 w-7" disabled={!onMoveUp} onClick={onMoveUp}><ArrowUp className="h-3.5 w-3.5" /></Button>
          <Button size="icon" variant="outline" className="h-7 w-7" disabled={!onMoveDown} onClick={onMoveDown}><ArrowDown className="h-3.5 w-3.5" /></Button>
        </div>
        <div className="flex-1 space-y-2">
          ...
          <div className="flex gap-2 items-center">
            <Select value={defaultStatus} onValueChange={(v) => setDefaultStatus(v as Step['default_status'])}>
