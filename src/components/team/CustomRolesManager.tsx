...
export const CustomRolesManager = ({ onRolesChange }: Props) => {
  ...
  const createRole = async () => {
    ...
    if (error) {
      ...
      setName(''); setDescription(''); setColor('#3b82f6'); setPermissions([]);
      setOpen(false);
      fetchRoles();
      onRolesChange?.();
    }
            ...
            {roles.map(r => (
              <div key={r.id} className="flex items-center justify-between bg-muted/50 rounded-lg px-3 py-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-[10px] text-muted-foreground">{(r.permissions as string[]).length} perms</span>
                </div>
