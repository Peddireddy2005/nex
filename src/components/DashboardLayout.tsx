...
import { applySessionPolicy, type SessionRow } from '@/lib/sessionPolicy';
...
const DashboardLayout: React.FC = () => {
  ...
  // Track user session with detailed device info + strict single-device enforcement
  useEffect(() => {
    if (!user) return;
    ...
    const upsertSession = async () => {
      ...
      } as any, { onConflict: 'user_id,session_id' }).select();
    };

    // Apply device policy: strict single-device, or multi-device with per-device dedupe
    const enforceSingleDevice = async () => {
      const { data: setting } = await supabase
        .from('user_device_settings')
        .select('allow_multi_device')
        .eq('user_id', user.id)
        .maybeSingle();
      const allowMulti = (setting as any)?.allow_multi_device === true;
      const { data: rows } = await supabase
        .from('user_sessions')
        .select('id, user_id, session_id, device_info, last_active_at')
        .eq('user_id', user.id);
    ...
    const interval = setInterval(async () => {
      await supabase.from('user_sessions').update({ last_active_at: new Date().toISOString() } as any).eq('user_id', user.id).eq('session_id', sessionId);
      // Re-check strict policy periodically so admin toggles take effect mid-session
      await enforceSingleDevice();
    }, 60000);
