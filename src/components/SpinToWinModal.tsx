...
// Auto-close if the popup sits untouched for 60s. Shown at most once every 7 days (per browser).
const AUTO_CLOSE_MS = 60 * 1000;
const REAPPEAR_INTERVAL_MS = 7 * 24 * 60 * 60 * 1000;
const LAST_SHOWN_KEY = "nexubotics_spinwin_last_shown";
...
export default function SpinToWinModal() {
  ...
  // Show once every 7 days per browser — appears right away if due.
  useEffect(() => {
    let lastShown: number | null = null;
    try {
      const stored = window.localStorage.getItem(LAST_SHOWN_KEY);
      lastShown = stored ? Number(stored) : null;
    } catch {
      // localStorage unavailable (private mode, blocked storage, etc.) — fall back to always eligible
    }

    const dueToShow = !lastShown || Date.now() - lastShown >= REAPPEAR_INTERVAL_MS;
    if (!dueToShow) return;
  ...
  // Auto-close after 60s of no interaction — but never while spinning or showing a result.
  useEffect(() => {
    if (!isOpen || spinning || view === "success") return;
    const timer = setTimeout(() => setIsOpen(false), AUTO_CLOSE_MS);
    return () => clearTimeout(timer);
  }, [isOpen, spinning, view]);
  ...
  const drawWheel = (angle: number) => {
    ...
    if (!ctx) return;
    // Render at devicePixelRatio so the wheel stays crisp on mobile/retina
    // screens instead of the browser upscaling a low-res canvas.
    const dpr = window.devicePixelRatio || 1;
    const target = Math.round(wheelSize * dpr);
    if (canvas.width !== target || canvas.height !== target) {
      canvas.width = target;
      canvas.height = target;
    }
    const W = wheelSize, R = W / 2;
    const scale = W / 320;
  ...
  useEffect(() => { if (isOpen) drawWheel(currentAngleRef.current); }, [wheelSize, isOpen]);
  ...
  // Keep the spin hub at a real tap target on small screens (44px per
  // WCAG 2.5.5), even though 20% of the smallest wheel size would be 36px.
  const hubSize = Math.max(44, Math.round(wheelSize * 0.2));
    ...
    <div style={{
      ...
      <div style={{ position:"relative", width:wheelSize, height:wheelSize }}>
        ...
        <canvas ref={canvasRef}
          style={{ position:"relative", zIndex:2, borderRadius:"50%", display:"block", width: wheelSize, height: wheelSize, filter:"drop-shadow(0 0 18px rgba(0,198,255,.3))" }} />
  ...
  const formBlock = (
    <div style={{
      ...
      {[
        { label:"Name / Company", type:"text", value:name, set:setName, err:nameErr, setErr:setNameErr, placeholder:"Krish Sharma / Nexubotics", autoComplete:"name", inputMode:"text" as const },
        { label:"Phone Number", type:"tel", value:phone, set:setPhone, err:phoneErr, setErr:setPhoneErr, placeholder:"+91 99999 99999", autoComplete:"tel", inputMode:"tel" as const },
        { label:"Email Address", type:"email", value:email, set:setEmail, err:emailErr, setErr:setEmailErr, placeholder:"krish@nexubotics.com", extra:() => setFormError(""), autoComplete:"email", inputMode:"email" as const },
      ].map(({ label, type, value, set, err, setErr, placeholder, extra, autoComplete, inputMode }) => (
