...
const SERVICE_OPTIONS = [
  'AI Ads', 'Marketing Automation', 'Website Build',
  'CRM Setup', 'Social Media', 'Custom',
];
...
export function NewClientWizard({ open, onOpenChange, onCreated }: Props) {
  const { user } = useAuth();
  ...
  const reset = () => {
    setStep(1); setName(''); setContactName(''); setEmail(''); setPhone('');
    setCompany(''); setServiceTypes([]); setBudget('');
    setKickoffDate(new Date().toISOString().split('T')[0]); setNotes('');
    setAutoGenerate(true); setErrors({});
  };
  ...
  const next = async () => {
    if (step === 1) {
      ...
      if (dup) { setErrors({ email: 'Email already exists' }); return; }
      setStep(2);
    } else if (step === 2) {
    ...
    try {
      ...
      const { data: client, error: cErr } = await supabase
        ...
        .insert({
          ...
          notes: notes || null, service_type: serviceLabel,
      ...
      await supabase.from('activity_logs').insert({
        user_id: user.id, action: 'client_created', entity_type: 'client',
        entity_id: (client as any).id,
        metadata: { name, project_id: (project as any).id } as any,
      });
      ...
      if (autoGenerate) {
        ...
        try {
          const { data: ai } = await supabase.functions.invoke('ai-project-generator', {
            body: {
              action: 'generate_tasks', clientId: (client as any).id,
              clientName: name, serviceType: serviceLabel,
            },
          });
          if (ai?.tasks?.length) {
            // Link tasks to project
            const ids = ai.tasks.map((t: any) => t.id);
            await supabase.from('tasks').update({ project_id: (project as any).id } as any).in('id', ids);
          }
        } catch (aiErr) {
      ...
      logError({
        ...
        action: 'client_onboarding', component: 'NewClientWizard',
        severity: 'error',
      });
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => {
      // Prevent close on outside click (require explicit cancel)
      if (!o && !saving) {
        onOpenChange(false);
        reset();
      } else if (o) {
          ...
          <div className="space-y-3">
            <div className="rounded-lg border bg-muted/30 p-4 space-y-2">
              ...
              <div className="text-sm space-y-1">
                <p><span className="text-muted-foreground">Client:</span> {name}</p>
                <p><span className="text-muted-foreground">Email:</span> {email}</p>
                {company && <p><span className="text-muted-foreground">Company:</span> {company}</p>}
                <p><span className="text-muted-foreground">Services:</span></p>
                <div className="flex flex-wrap gap-1">
                  {serviceTypes.map(s => <Badge key={s} variant="secondary" className="text-[10px]">{s}</Badge>)}
                </div>
                <p><span className="text-muted-foreground">Kickoff:</span> {kickoffDate}</p>
                {budget && <p><span className="text-muted-foreground">Budget:</span> ${parseFloat(budget).toLocaleString()}</p>}
              </div>
            </div>
            ...
            <p className="text-xs text-muted-foreground">
              On submit: a project named <strong>{name} — {serviceTypes.join(' + ')}</strong> will be created with a 30-day default deadline.
            </p>
          </div>
        )}
        ...
        <DialogFooter className="flex-row justify-between sm:justify-between">
          ...
          {step < 3 ? (
            ...
            <Button onClick={submit} disabled={saving}>
              {saving ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Creating…</> : 'Create & Launch'}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
