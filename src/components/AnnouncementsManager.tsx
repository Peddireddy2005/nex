...
export function AnnouncementsManager() {
  ...
  const create = async () => {
    ...
    setTitle(''); setBody(''); setSeverity('info'); setOpen(false);
    fetchItems();
  };
            ...
            {items.map(a => (
              <li key={a.id} className="flex items-center justify-between gap-2 text-sm p-2 rounded-md hover:bg-muted/50">
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground capitalize">{a.severity} · {new Date(a.starts_at).toLocaleDateString()}</p>
                </div>
