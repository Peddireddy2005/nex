...
export function ProjectAIPanel({ projectId, projectName, serviceTypes, onTasksCreated }: Props) {
  ...
  const generateTasks = async () => {
    ...
    if (data?.tasks?.length) {
      ...
      await supabase.from('tasks').update({ project_id: projectId } as any).in('id', ids);
