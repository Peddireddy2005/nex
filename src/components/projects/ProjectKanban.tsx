...
export function ProjectKanban({ projectId, canEdit }: { projectId: string; canEdit: boolean }) {
  ...
  const onDrop = async (e: React.DragEvent, status: string) => {
    ...
    const { error } = await supabase.from('tasks').update({ status: status as any }).eq('id', dragId);
    if (error) { toast.error('Failed to move task'); load(); }
    setDragId(null);
  };

  const openCreate = (col: string) => {
    setNewCol(col); setTitle(''); setDescription(''); setPriority('medium'); setDeadline('');
    setShowCreate(true);
  };
  ...
  const createTask = async () => {
    ...
    const { error } = await supabase.from('tasks').insert({
      title, description: description || null, status: newCol as any,
      priority: priority as any, deadline: deadline || null,
  ...
  return (
    <>
        ...
        {COLUMNS.map(col => {
          ...
          return (
            <div key={col.id}
              ...
              className="glass rounded-xl p-2 min-h-[200px] liquid-sheen">
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          <DialogHeader><DialogTitle>New task in {COLUMNS.find(c => c.id === newCol)?.label}</DialogTitle></DialogHeader>
