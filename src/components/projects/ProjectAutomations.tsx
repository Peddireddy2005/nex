...
export function ProjectAutomations({ projectId, canEdit }: { projectId: string; canEdit: boolean }) {
  const { user } = useAuth();
  ...
  const load = async () => {
    ...
    const { data } = await supabase.from('automation_rules').select('*')
      .contains('trigger_config', { project_id: projectId })
  ...
  const create = async () => {
    ...
    if (rules.length >= 20) { toast.error('Maximum 20 rules per project'); return; }
    const { error } = await supabase.from('automation_rules').insert({
      name, trigger_type: trigger,
      trigger_config: { project_id: projectId } as any,
      actions: [{ type: action, config: actionConfig }] as any,
      conditions: [] as any,
      enabled: true,
      created_by: user.id,
    } as any);
    ...
    setShowCreate(false);
    setName(''); setActionConfig('');
    load();
  };

  const toggle = async (id: string, enabled: boolean) => {
    await supabase.from('automation_rules').update({ enabled } as any).eq('id', id);
    load();
  };
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          <Badge variant="secondary" className="text-[10px]">{rules.length}/20</Badge>
        </CardTitle>
        {canEdit && (
      ...
      <CardContent>
        {loading ? <p className="text-sm text-muted-foreground">Loading…</p> : rules.length === 0 ? (
          ...
          <ul className="space-y-2">
            {rules.map(r => {
              const actionLabel = ACTIONS.find(a => a.value === (r.actions as any)?.[0]?.type)?.label;
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input value={actionConfig} onChange={(e) => setActionConfig(e.target.value)}
                placeholder="e.g. notification message or status value" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowCreate(false)}>Cancel</Button>
            <Button onClick={create} disabled={!name.trim()}>Create Rule</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </Card>
