...
export function AnnouncementBanner() {
  ...
  useEffect(() => {
    const fetchActive = async () => {
      ...
      const { data } = await (supabase as any)
        ...
        .limit(5);
      const active = (data || []).filter((a: Announcement) => !a.ends_at || a.ends_at > nowIso);
      setItems(active);
    };
    fetchActive();
  ...
  const tone =
    visible.severity === 'warning' ? { icon: AlertTriangle, ring: 'border-warning/40 bg-warning/10 text-warning' }
    : visible.severity === 'success' ? { icon: CheckCircle2, ring: 'border-success/40 bg-success/10 text-success' }
    : visible.severity === 'critical' ? { icon: AlertTriangle, ring: 'border-destructive/40 bg-destructive/10 text-destructive' }
