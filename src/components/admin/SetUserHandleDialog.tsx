...
export function SetUserHandleDialog({ open, onOpenChange, userId, userName, currentHandle, onSaved }: Props) {
  const [value, setValue] = useState(currentHandle || '');
  const [busy, setBusy] = useState(false);

  useEffect(() => { setValue(currentHandle || ''); }, [currentHandle, open]);
  ...
  const save = async () => {
    ...
    const { error } = await (supabase as any)
      .from('profiles')
      .update({ user_handle: handle || null })
      .eq('id', userId);
    setBusy(false);
