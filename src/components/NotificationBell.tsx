...
const defaultPrefs: Prefs = { muted: false, sound: true, showRead: true };
...
export function NotificationBell() {
  ...
  const clearOld = async () => {
    ...
    const { error } = await supabase.from('notifications').delete()
      .eq('user_id', user.id).eq('is_read', true).lt('created_at', cutoff);
    if (error) {
        ...
        {showSettings ? (
          <div className="p-3 space-y-3">
            ...
            <Button variant="outline" size="sm" className="w-full" onClick={clearOld}>
              <Trash2 className="h-3.5 w-3.5 mr-2" />Clear read older than 7 days
            </Button>
            <Button variant="outline" size="sm" className="w-full" onClick={clearAllRead}>
              <Trash2 className="h-3.5 w-3.5 mr-2" />Clear all read
            </Button>
          </div>
        ) : (
          <div className="max-h-96 overflow-y-auto">
            {visible.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">
                {items.length === 0 ? 'No notifications' : 'No unread notifications'}
              </p>
            ) : (
                ...
                {visible.map(n => (
                  <li key={n.id}>
                    <button onClick={() => handleClick(n)}
                      className={`w-full text-left p-3 hover:bg-accent transition-colors ${!n.is_read ? 'bg-primary/5' : ''}`}>
